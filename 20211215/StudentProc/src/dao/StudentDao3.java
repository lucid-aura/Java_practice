...
public class StudentDao3 {
	...
	StudentDto3 student[] = new StudentDto3[20]; // 20개의 공간을 가지는 StudentDto 타입의 변수 student배열 선언(객체 생성x)
	private int count; // 입력된 개수를 저장할 int 타입 변수 count 선언
	
	public StudentDao3() { // 기본 생성자 : count만 0으로 초기화 한다. 
		count = 0;
	}
	...
	public void fileRead() { // 파일 읽기
		try {
			...
			while((str = br.readLine()) != null) {
				...
				student[count] = new StudentDto3(Integer.parseInt(split[0]), split[1], Double.parseDouble(split[2]), Integer.parseInt(split[3]), Integer.parseInt(split[4]));
				count++;
			}		
		} catch (Exception e) {			
